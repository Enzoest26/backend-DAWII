...
public interface IComidaRepository extends JpaRepository<Comida, String> {
	...
	@Query("SELECT COUNT(c) FROM Comida c INNER JOIN Postre p ON p.idComida = c.idComida WHERE c.estadoComida = 1")
	Integer contarTotalPostres();
	
	@Query("SELECT COUNT(c) FROM Comida c INNER JOIN Sandwich s ON s.idComida = c.idComida WHERE c.estadoComida = 1")
	Integer contarTotalSandwiches();
